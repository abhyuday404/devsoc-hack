...
1. **Understand the PDF**: Call getPdfMetadata to learn the page count, file size, and see the first page text. Note the pdfPath and jobDir returned — you will need them for subsequent tool calls.
...
export async function processPdf(pdfKey) {
...
Start by calling getPdfMetadata with the pdfKey and jobDir above.`,
        ...
        const lastStepToolResults = result.steps
            .flatMap((step) => step.toolResults)
            .filter(Boolean);
        // Look for a successful upload result
        const uploadResult = lastStepToolResults.find((tr) => tr !== null &&
            typeof tr === "object" &&
            "key" in tr &&
            "success" in tr &&
            tr.success === true);
