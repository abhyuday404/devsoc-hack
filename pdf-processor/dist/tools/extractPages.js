...
export const extractPages = tool({
    description: "Extract text and table data from specific pages of the PDF. " +
    ...
    parameters: z.object({
        ...
        pageNumbers: z
            ...
            .min(1)
            .max(20)
            .describe("Array of 0-based page indices to extract. " +
            "For example, [0, 1, 4, 5] extracts pages 1, 2, 5, and 6."),
    }),
    execute: async ({ pdfPath, pageNumbers }) => {
        ...
        const result = await runHelperScript("extract_pages.py", [pdfPath, pageNumsArg], 20_000);
        if (!result.success) {
            ...
            throw new Error(`Failed to extract pages: ${result.error ?? "Unknown error"}`);
        }
        try {
            ...
            throw new Error(`Failed to parse Python output as JSON: ${String(parseError)}`);
        }
    },
});
