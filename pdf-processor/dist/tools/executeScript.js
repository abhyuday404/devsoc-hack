...
analyze the problem and generate a corrected script.`,
    parameters: z.object({
        script: z
            .string()
            .describe("The full Python script to execute. Do NOT include the PDF_PATH or OUTPUT_DIR assignments — they are injected automatically."),
        pdfPath: z
            .string()
    ...
    execute: async ({ script, pdfPath, jobDir }) => {
        ...
        if (!outputFilename) {
            ...
            return {
                success: false,
                error: "Script exited successfully but did not print an output filename and no CSV file was found in the output directory.",
                stdout: result.output.slice(0, 1000),
            };
        }
 ...
 */
async function findCsvInDir(dir) {
    try {
 ...
 */
async function buildSuccessResult(jobDir, csvFilename) {
