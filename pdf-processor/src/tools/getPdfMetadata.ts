...
export const getPdfMetadata = tool({
  ...
  execute: async ({ pdfKey, jobDir }): Promise<PdfMetadata> => {
    ...
    if (!result.success) {
      ...
      throw new Error(
        `Failed to extract PDF metadata: ${result.error ?? "unknown error"}`,
      );
    }
    ...
    try {
      ...
      throw new Error(
        `Failed to parse metadata output: ${result.output.slice(0, 500)}`,
      );
    }
